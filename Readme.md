...
## RESULTS
Results for our analysis can be viewed in the MainMLNotebook.ipynb
The reason for overfitting might be that most likely, one of our variance parameters is converging to zero. Note that inference for variance parameters is not meaningful when the point estimate is at or near zero.
